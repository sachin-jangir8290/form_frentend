...
import GBPFormModal from '../GBPFormModal';
...
const FloatingButton = () => {
  ...
  const toggleForm = () => {
    ...
    if (!isFormOpen) {
      ...
      }).catch(err => console.error('Button click tracking failed:', err));
    }
  ...
  const closeForm = () => {
    ...
    fetch("http://localhost:5000/api/form-close", {
      ...
      body: JSON.stringify({
        ...
        userAgent: navigator.userAgent,
        referrer: document.referrer || 'Direct',
        action: 'close_without_submit'
      }),
    }).catch(err => console.error('Form close tracking failed:', err));
  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      if (!buttonRef.current || !leftEyeRef.current || !rightEyeRef.current) return;
      ...
      leftEyeRef.current.style.transform = `rotate(${leftAngle}deg)`;
      rightEyeRef.current.style.transform = `rotate(${rightAngle}deg)`;
    };
  ...
  return (
    <>
      ...
      <div
        ...
        onClick={toggleForm}
        onMouseEnter={() => {
          console.log('Mouse entered robot button');
          setIsFadingOut(false);
          setShowMessage(true);
        }}
        onMouseLeave={() => {
          console.log('Mouse left robot button');
          setIsFadingOut(true);
          setTimeout(() => {
        ...
        aria-label="Open GBP Suspension Checker"
      >
        ...
        {/* Hover Message */}
        {showMessage && (
          <div className={`robot-tooltip ${isFadingOut ? 'fade-out' : ''}`}>
            <div className="robot-tooltip-content">
              <span className="robot-tooltip-text">Hi! Click me to check your GBP status! 🤖</span>
              <div className="robot-tooltip-arrow"></div>
            </div>
          </div>
        )}

        {/* Debug: Always visible tooltip for testing */}
        <div className="robot-tooltip debug-tooltip" style={{top: '-90px'}}>
          <div className="robot-tooltip-content">
            <span className="robot-tooltip-text">DEBUG: Always visible 🤖</span>
            <div className="robot-tooltip-arrow"></div>
          </div>
        </div>
      </div>
      ...
      {/* GBP Form Modal */}
      <GBPFormModal isOpen={isFormOpen} onClose={closeForm} />
    </>
  );
};
